...
from libs.tools import segment_video_labels, gen_label, generate_segment_features, create_logits, generate_freq_features
...
def train(
    ...
    for sample in tqdm(train_loader):
        ...
        output_cls, output_bound, output_feature, freq_feature, logit_scale = model(x, mask, joint_embeddings_graph)

        # get segment boundary and cls
        ...
        action_freq = generate_freq_features(freq_feature, t_segment, device, dataset) #action spectrum
        ...
        logits_per_image, logits_per_text = create_logits(action_features[0], action_embedding, logit_scale[0])  # sim matrix
        ground_truth = torch.tensor(label_g, dtype=action_features[0].dtype, device=device)  # GT matrix
        ...
        # Adjacent Action Discrepancy Loss
        freq_batch = len(action_freq)
        loss_freq = 0
        for action_freq_instance in action_freq:
            diff = action_freq_instance[1:] - action_freq_instance[:-1]
            # diff = torch.norm(diff, p=1, dim=-1)
            diff = torch.mean(torch.abs(diff), dim=-1) * 100
            loss_freq_instance = -torch.mean(torch.log(torch.tanh(diff)))
            loss_freq += loss_freq_instance / freq_batch
