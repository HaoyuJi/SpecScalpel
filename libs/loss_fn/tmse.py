...
class TMSE(nn.Module):
    ...
    def forward(self, preds: torch.Tensor, gts: torch.Tensor) -> torch.Tensor:
        ...
        for pred, gt in zip(preds, gts):
            pred = pred[:, torch.where(gt != self.ignore_index)[0]] #  ignore_index
...
class GaussianSimilarityTMSE(nn.Module):
    ...
    def __init__(
        ...
        self.ignore_index = ignore_index
        self.mse = nn.MSELoss(reduction="none")
        self.sigma = sigma # Gaussian Similarity
    ...
    def forward(
        ...
        for pred, gt, sim in zip(preds, gts, sim_index):
            ...
            diff = sim[:, 1:] - sim[:, :-1]
            similarity = torch.exp(-torch.norm(diff, dim=0) / (2 * self.sigma ** 2)) #norm（C，T-1）->（T-1）
